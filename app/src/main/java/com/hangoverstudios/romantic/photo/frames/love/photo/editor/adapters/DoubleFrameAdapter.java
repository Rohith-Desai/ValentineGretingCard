...
import com.hangoverstudios.romantic.photo.frames.love.photo.editor.activities.DoubleFrameEditActivity;
import com.hangoverstudios.romantic.photo.frames.love.photo.editor.activities.EditScreen;
...
public class DoubleFrameAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public DoubleFrameAdapter(ArrayList<Object> mData, Context CONTEXT, int type, String cat) {
        this.mData = mData;
        this.mContext = CONTEXT;
        this.type = type;
        this.category = cat;
        if (category.equals("anniversary")) {
            if (DataHolder.getDataHolder().getdAnniversary() != null) {
                downloadList = DataHolder.getDataHolder().getdAnniversary();
            }
        } else if (category.equals("birthday")) {
            if (DataHolder.getDataHolder().getdBirthday() != null) {
                downloadList = DataHolder.getDataHolder().getdBirthday();
            }
        } else if (category.equals("flower")) {
            if (DataHolder.getDataHolder().getdFlowers() != null) {
                downloadList = DataHolder.getDataHolder().getdFlowers();
            }
        } else if (category.equals("quote")) {
            if (DataHolder.getDataHolder().getdQuotes() != null) {
                downloadList = DataHolder.getDataHolder().getdQuotes();
            }
        } else {
            if (DataHolder.getDataHolder().getDoubleFramesList() != null) {
                downloadList = DataHolder.getDataHolder().getDoubleFramesList();
            }
        }
    }
                        ...
                        public void onClick(View view) {
                            if (finalMediaStorageDir != null) {
                                if (!finalMediaStorageDir.exists()) {
                                    ...
                                    bundle.putString(FirebaseAnalytics.Param.CONTENT_TYPE,   "doublelove/" + category + "/" + paths[paths.length - 1]);
                                    if (mFirebaseAnalytics != null) {
                                        mFirebaseAnalytics.logEvent("ROMANTIC_SELECTION", bundle);
                                    }

                                    mContext.startActivity(new Intent(mContext, EditScreen.class).putExtra("PATH", downloadList.get(position).toString())
                                            .putExtra("FROM", "Double"));
                                }
                            }
                        }
                    });
